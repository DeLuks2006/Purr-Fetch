...
void display(void){
  ...
  printf("(\\_/)\t\033[0;33m uptime: %li\n\033[0;0m", uptime(&uptime_buf));// uptime  - orange
  printf("(oᴥo)\t\033[0;31m shell: %s\n\033[0;0m", shell(&shell_buf)); // shell   - red
  printf("|U°U|\t\033[0;35m distro: %s\n\033[0;0m", distro(&distro_buf)); // distro  - purple
  printf("|   |\t\033[0;34m hostname: %s\n\033[0;0m", hostname(&hostname_buf)); // host    - blue
  printf("'U_U'\t\033[0;36m kernel: %s\n\033[0;0m", kernel(&kernel_buf)); // kernel  - cyan
  printf("  U\n");
